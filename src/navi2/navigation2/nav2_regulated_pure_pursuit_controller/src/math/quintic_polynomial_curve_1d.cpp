...
namespace math
{
QuinticPolynomialCurve1d::QuinticPolynomialCurve1d(const std::array<double, 3>& start, const std::array<double, 3>& end,
                                                   const double param)
  : QuinticPolynomialCurve1d(start[0], start[1], start[2], end[0], end[1], end[2], param)
{
}

QuinticPolynomialCurve1d::QuinticPolynomialCurve1d(const double x0, const double dx0, const double ddx0,
...
double QuinticPolynomialCurve1d::ParamLength() const
{
    return param_;
}

std::string QuinticPolynomialCurve1d::String() const
{
    // boost::format fmt("5-Curve1d: coef(%|1|, %|1|, %|1|, %|1|, %|1|, %|1|),"
    //                   "param(%|1|), "
    //                   "start(%|1|, %|1|, %|1|), "
    //                   "end(%|1|, %|1|, %|1|)");
    // return (fmt % coef_[0] % coef_[1] % coef_[2] % coef_[3] % coef_[4] % coef_[5] % param_ % start_condition_[0] %
    //         start_condition_[1] % start_condition_[2] % end_condition_[0] % end_condition_[1] % end_condition_[2])
    //     .str();
    return "";
}

void QuinticPolynomialCurve1d::ComputeCoefficients(const double x0, const double dx0, const double ddx0,
                                                   const double x1, const double dx1, const double ddx1, const double p)
{
    if (p <= 0.0)
    {
        std::cout << "p must bigger than 0.0. " << std::endl;
