...
double PathHandler::getCostmapMaxExtent() const
{
    const double max_costmap_dim_meters =
        std::max(costmap_ros_->getCostmap()->getSizeInMetersX(), costmap_ros_->getCostmap()->getSizeInMetersY());
...
nav_msgs::msg::Path PathHandler::transformGlobalPlan(nav_msgs::msg::Path&                   path,
                                                     const geometry_msgs::msg::PoseStamped& pose)
{
    ...
    // Find points definitely outside of the costmap so we won't transform them.
    auto transformation_end = std::find_if(transformation_begin, end(path.poses), [&](const auto& global_plan_pose) {
        return euclidean_distance(robot_pose, global_plan_pose) > max_transform_dist;
    });
    ...
    // Remove the portion of the global plan that we've already passed so we don't
    // process it on the next iteration (this is called path pruning)
    path.poses.erase(begin(path.poses), transformation_begin);
    // global_path_pub_->publish(transformed_plan);
...
bool PathHandler::transformPose(const std::string frame, const geometry_msgs::msg::PoseStamped& in_pose,
