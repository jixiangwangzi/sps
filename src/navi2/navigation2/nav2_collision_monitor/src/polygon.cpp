...
bool Polygon::configure()
{
  ...
  if (!footprint_topic.empty()) {
    footprint_sub_ = std::make_unique<FootprintSubscriber>(
...
// TODO: polygon 
void Polygon::updatePolygonStop(const Velocity & velocity,const double max_vel)
{
  //std::size_t new_size = poly_.size();
  double ratio = velocity.x / max_vel;
  RCLCPP_INFO(rclcpp::get_logger("polygon"), "ratio = %f",ratio);
  for(std::size_t i = 0;i < polygon_.points.size(); i++){
      polygon_.points[i].x = poly_[i].x * ratio ;
      polygon_.points[i].y = poly_[i].y;
      if(polygon_.points[i].x < min_box_ && polygon_.points[i].x > 0){
        //min box 
        polygon_.points[i].x = min_box_;        
      }
      if(velocity.x == 0.0){
        polygon_.points[i].x = 0.0;
        polygon_.points[i].y = 0.0;
      }
  }
}
// TODO: Polygon
void Polygon::updatePolygonSlowdown(const Velocity & velocity,const double max_vel)
{
  //std::size_t new_size = poly_.size();
  double ratio = velocity.x / max_vel;
  RCLCPP_INFO(rclcpp::get_logger("polygon"), "ratio slowdown= %f",ratio);
  for(std::size_t i = 0;i < polygon_.points.size(); i++){
      polygon_.points[i].x = poly_[i].x * ratio ;
      polygon_.points[i].y = poly_[i].y;
      if(polygon_.points[i].x < min_box_ && polygon_.points[i].x > 0){
...
void Polygon::updatePolygonStopNew(const Velocity & velocity,const double max_vel)
{
  updatePolygon();
  ...
  double time = 0.0;
  for (time = 0.0; time <= time_before_collision_; time += simulation_time_step_) {
    ...
    record_collision_pose_ = pose;
    
    // Transform collision_points to the frame concerned with current robot pose
    //points_transformed = collision_points;// sonor data
  ...
  if(time == time_before_collision_){
    //no_collision
  }
...
void Polygon::updatePolygon()
{
  if (footprint_sub_ != nullptr) {
    ...
    for (std::size_t i = 0; i < new_size; i++) {
      ...
      //RCLCPP_INFO(rclcpp::get_logger("polygon"),"polyoint:[%f,%f]",polygon_.points[i].x,polygon_.points[i].y);
    }
  }
}
  ...
  const Velocity & velocity)
{
  ...
  for (double time = 0.0; time <= time_before_collision_; time += simulation_time_step_) {
    //RCLCPP_INFO(rclcpp::get_logger("polygon"),"double time == %f",time);
    // Shift the robot pose towards to the vel during simulation_time_step_ time interval
    // NOTE: vel is changing during the simulation
    projectState(simulation_time_step_, pose, vel);
    Point p;
    ...
    if(time == 0.0 || (time <= time_before_collision_ && time >= (time_before_collision_ - 0.1)) || time < 3.0){
      if(time == 0.0 || (time <= time_before_collision_ && time >= (time_before_collision_ - 0.1))){
        //RCLCPP_INFO(rclcpp::get_logger("polygon"),"time == %f",time);
      }
      for(double angle = 0.0; angle <= 2 * M_PI; angle += angle_increment){
        p.x = 0.28 * std::cos(angle + M_PI_2) + pose.x;
        p.y = 0.28 * std::sin(angle + M_PI_2) + pose.y;
        p_s.x = p.x;
        p_s.y = p.y;
        polygon_.points.push_back(p_s);
      } 
    }
    polygon_.points.push_back(polygon_.points.back());
    // }else{
    //     p.x = 0.28 * std::cos(M_PI_2) + pose.x;
    //     p.y = 0.28 * std::sin(M_PI_2) + pose.y;
    //     p_s.x = p.x;
    //     p_s.y = p.y;
    //     polygon_.points.push_back(p_s);
    // }
...
bool Polygon::getCommonParameters(std::string & polygon_pub_topic)
{
  ...
  try {
    ...
    if (action_type_ == SLOWDOWN || action_type_ == STOP) {
      nav2_util::declare_parameter_if_not_declared(
        node, polygon_name_ + ".min_box", rclcpp::ParameterValue(0.3));
      min_box_ = node->get_parameter(polygon_name_ + ".min_box").as_double();
    }
    ...
    if (visualize_) {
      ...
      //RCLCPP_INFO(rclcpp::get_logger("polycpp"),"poly pub topic : %s",polygon_pub_topic.c_str());
    }
