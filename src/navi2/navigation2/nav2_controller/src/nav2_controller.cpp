...
: LifecycleNode("controller_server", "", true, options),
  ...
  distance_remaining_(0.0)
{
  ...
  declare_parameter("parking_dist_threshold", rclcpp::ParameterValue(2.0));
  declare_parameter("failure_tolerance", rclcpp::ParameterValue(0.0));
  declare_parameter("navi_type_topic", rclcpp::ParameterValue("/navi_type"));      // add

  get_parameter("navi_type_topic", navi_type_topic_);                        // add
  ...
  costmap_node_options_arguments.push_back(std::string("__ns:=") + nav2_util::add_namespaces(std::string{get_namespace()}, "local_costmap"));
  costmap_node_options_arguments.push_back("--ros-args");
  costmap_node_options_arguments.push_back("-r");
  costmap_node_options_arguments.push_back("local_costmap:" + std::string("__node:=") + "local_costmap");
  if (!param_file.empty()) {
  ...
  for (const auto &i: costmap_node_options.arguments()) {
    RCLCPP_INFO(get_logger(), "local_costmap arguments: %s", i.c_str());
  }
  costmap_ros_ = std::make_shared<nav2_costmap_2d::Costmap2DROS>(
    "local_costmap", std::string{get_namespace()}, "local_costmap", costmap_node_options);

  // Launch a thread to run the costmap node
  costmap_thread_ = std::make_unique<nav2_util::NodeThread>(costmap_ros_);

  lift_access_state_ = 0;
}
...
ControllerServer::on_configure(const rclcpp_lifecycle::State & state)
{
  ...
  get_parameter("parking_dist_threshold", parking_dist_threshold_);
  RCLCPP_INFO(get_logger(), "Controller frequency set to %.4fHz, parking_dist_threshold : %f.", controller_frequency_, parking_dist_threshold_);
  ...
  //backup_controller_ = std::make_shared<nav2_backup_controller::BackupController>(node,"backup_controller",costmap_ros_->getTfBuffer(),costmap_ros_);
  //backup_controller_->configure();

  odom_sub_ = std::make_unique<nav_2d_utils::OdomSubscriber>(node);
  vel_publisher_ = create_publisher<geometry_msgs::msg::Twist>("cmd_vel", 1);
  end_pose_publisher_ = create_publisher<geometry_msgs::msg::PoseStamped>("end_pose", 1);
  distance_remaining_pub_ = create_publisher<std_msgs::msg::Float32>("distance_remaining",1);

  navi_type_sub_ = node->create_subscription<std_msgs::msg::Int32>(
        navi_type_topic_, rclcpp::SystemDefaultsQoS(),
        std::bind(&ControllerServer::naviTypeCallback, this, std::placeholders::_1));  // add
...
ControllerServer::on_activate(const rclcpp_lifecycle::State & state)
{
  ...
  end_pose_publisher_->on_activate();
  distance_remaining_pub_->on_activate();
  action_server_->activate();

  //backup_controller_->activate();

  // auto node = shared_from_this();
  // anti_collision_service_ = node->create_service<cm_msgs::srv::AutoMove>(
  //   "AutoMove",
  //   std::bind(&ControllerServer::AutoMove,this,
  //   std::placeholders::_1,std::placeholders::_2));
  // backup_thread_ = std::make_shared<std::thread>(std::bind(&ControllerServer::BackupThreadLoop, this));
...
ControllerServer::on_deactivate(const rclcpp_lifecycle::State & state)
{
  ...
  end_pose_publisher_->on_deactivate();
  distance_remaining_pub_->on_deactivate();
  //backup_controller_->deactivate();
...
ControllerServer::on_cleanup(const rclcpp_lifecycle::State & state)
{
  ...
  //backup_controller_->cleanup();

  // Release any allocated resources
  action_server_.reset();
  odom_sub_.reset();
  vel_publisher_.reset();
  end_pose_publisher_.reset();
...
void ControllerServer::computeControl()
{
 ...
 // if(backup_controller_->getAntiCollision()){
 //   RCLCPP_WARN_THROTTLE(get_logger(), *(clock_), 1000, "anti_collision is true!!!!");
 //   return;
 // }
  ...
  try {
    ...
    while (rclcpp::ok()) {
     ...
     // }
      ...
      // RCLCPP_WARN_THROTTLE(get_logger(), *(clock_), 1000, "distance_remaining_: %f", distance_remaining_);
      if (distance_remaining_ < parking_dist_threshold_) {
        RCLCPP_WARN_THROTTLE(get_logger(), *(clock_), 1000, "distance_remaining_: %f", distance_remaining_);
        if (isGoalReached()) {
...
void ControllerServer::setPlannerPath(const nav_msgs::msg::Path & path)
{
  ...
  //if (lift_access_state_ == 1 || lift_access_state_ == 3)
  //{
  //  controllers_[current_controller_]->setSpeedLimit(50.0, true);  // add
  //}else{
  //  controllers_[current_controller_]->setSpeedLimit(100.0, true);
  //}

  end_pose_ = path.poses.back();
  end_pose_.header.frame_id = path.header.frame_id;
  if (end_pose_publisher_->is_activated() && end_pose_publisher_->get_subscription_count() > 0) {
  ...
  //   RCLCPP_INFO(
  //   get_logger(), "The %zu path pose(x: %f, y: %f, yaw: %f).", curr_idx,
  //   path.poses[curr_idx].pose.position.x, path.poses[curr_idx].pose.position.y,
  //   tf2::getYaw(path.poses[curr_idx].pose.orientation));
  // }
...
void ControllerServer::computeAndPublishVelocity()
{
  ...
  try {
    ...
    //RCLCPP_INFO(rclcpp::get_logger("nav2_contrelloer"),"cmd_vel-v,w: (%f,%f)",cmd_vel_2d.twist.linear.x,cmd_vel_2d.twist.angular.z);
    last_valid_cmd_time_ = now();
  } catch (nav2_core::PlannerException & e) {
  ...
  distance_remaining_ = nav2_util::geometry_utils::calculate_path_length(current_path_, find_closest_pose_idx());
  if (distance_remaining_pub_->is_activated() && distance_remaining_pub_->get_subscription_count() > 0) {
    std_msgs::msg::Float32 dist;
    dist.data = distance_remaining_;
    distance_remaining_pub_->publish(dist);
  }
  feedback->distance_to_goal = distance_remaining_;
...
bool ControllerServer::getRobotPose(geometry_msgs::msg::PoseStamped & pose)
{
  ...
  pose = current_pose;
  // RCLCPP_INFO(
  //   get_logger(), "Current robot pose(x: %f, y: %f, yaw: %f).",
...
void ControllerServer::naviTypeCallback(const std_msgs::msg::Int32::ConstSharedPtr msg)  // add
{
    lift_access_state_ = msg->data;

    RCLCPP_INFO(get_logger(), "Receive access lift type : %d", lift_access_state_);
}
