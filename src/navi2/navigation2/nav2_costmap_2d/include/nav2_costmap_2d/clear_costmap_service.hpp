...
public:
  ...
  /**
   * @brief  Clears the region outside of a user-specified area 
   */
  void clearAppointedLayers();
...
private:
  ...
  rclcpp::Service<nav2_msgs::srv::ClearEntireCostmap>::SharedPtr clear_appointed_service_;
  /**
   * @brief Callback to clear costmap
   */
  void clearAppointedCallback(
    const std::shared_ptr<rmw_request_id_t> request_header,
    const std::shared_ptr<nav2_msgs::srv::ClearEntireCostmap::Request> request,
    const std::shared_ptr<nav2_msgs::srv::ClearEntireCostmap::Response> response);
