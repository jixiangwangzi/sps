  ...
  const seconds timeout)
{
  ...
  return change_state_.invoke(request, timeout).get();
}
  ...
  std::uint8_t transition)
{
  ...
  request->transition.id = transition;
  bool res = change_state_.invoke(request, response);
  RCLCPP_INFO(node_->get_logger(), "LifecycleServiceClient::change_state res: %d", (int) res);
  return res;
}
...
uint8_t LifecycleServiceClient::get_state(
  const seconds timeout)
{
  if (timeout != std::chrono::seconds::max() && !get_state_.wait_for_service(timeout)) {
    RCLCPP_ERROR(node_->get_logger(), "Service %s is not available.", get_state_.get_service_name().c_str());
    // RCLCPP_INFO(node_->get_logger(), "%s service client: waiting for service to appear...", get_state_.get_service_name().c_str());
