...
public:
  ...
  using ClientT = nav2_msgs::action::NavigateThroughPoses;
  using ActionServer = nav2_util::SimpleActionServer<ActionT>;
  using ActionClient = rclcpp_action::Client<ClientT>;
  using WaypointTasskExecutorMap = std::unordered_map<std::string, pluginlib::UniquePtr<nav2_core::WaypointTaskExecutor>>;
  /**
...
protected:
  ...
  ActionClient::SharedPtr nav_through_poses_client_;
  rclcpp::CallbackGroup::SharedPtr callback_group_;
  std::unique_ptr<nav2_util::NodeThread> action_client_thread_;
  std::shared_future<rclcpp_action::ClientGoalHandle<ClientT>::SharedPtr> future_goal_handle_;
  rclcpp_action::ClientGoalHandle<ClientT>::SharedPtr nav_through_poses_goal_handle_;
  ...
  WaypointTasskExecutorMap waypoint_task_executors_;
  std::unordered_map<uint8_t, std::string> waypoint_type_map_waypoint_task_executor_type_;
  std::vector<std::string> default_ids_;
  std::vector<std::string> default_types_;
  std::vector<std::string> waypoint_task_executor_ids_;
  std::vector<std::string> waypoint_task_executor_types_;
};
