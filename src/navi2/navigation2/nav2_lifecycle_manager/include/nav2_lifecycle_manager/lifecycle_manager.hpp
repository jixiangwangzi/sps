...
protected:
  ...
  /**
   * @brief For a node, check state and try to fix the right state
   */
  bool checkStateForNode(
  ...
  std::chrono::milliseconds respawn_duration_;

  // A map of all nodes to check bond connection
  std::map<std::string, std::shared_ptr<bond::Bond>> bond_map_;

  // A mutex to ensure bond_map sync.
  std::mutex bond_map_mutex_;

  // Time out of change state for node.
  const std::chrono::seconds timeout_ = 1s;
  ...
  // A map of the primary states code to primary states name
  std::unordered_map<std::uint8_t, std::string> primary_states_code_to_name_map_;

  // A map of the transitions code to transitions name
  std::unordered_map<std::uint8_t, std::string> transitions_code_to_name_map_;
