  ...
  const std::shared_ptr<nav2_costmap_2d::Costmap2DROS> & costmap_ros)
{
  ...
  nav2_util::declare_parameter_if_not_declared(
    node, plugin_name_ + ".enb_find_close_pose", rclcpp::ParameterValue(false));
  ...
  node->get_parameter(plugin_name_ + ".enb_find_close_pose", enb_find_close_pose_);
  node->get_parameter("general_goal_checker.xy_goal_tolerance", xy_goal_tolerance_);
  node->get_parameter("general_goal_checker.yaw_goal_tolerance", yaw_goal_tolerance_);
  RCLCPP_WARN(
    logger_, "enb_find_close_pose : %d, xy_goal_tolerance : %fm, yaw_goal_tolerance : %frad.",
    enb_find_close_pose_, xy_goal_tolerance_, yaw_goal_tolerance_);
  ...
  nav2_core::GoalChecker * goal_checker)
{
  if (path_updated_) {
    ...
    try {
      /* @test for partols
      double path_len = nav2_util::geometry_utils::calculate_path_length(current_path_);
      double angular_distance_to_heading = 0.;
      double temp_angular_threshold = angular_dist_threshold_;
      if (path_len < xy_goal_tolerance_) {
        // RCLCPP_WARN(
        //   logger_, "path length %.2fm less than %fm.", path_len, xy_goal_tolerance_);
        geometry_msgs::msg::Pose sampled_pt_base =
          transformPoseToBaseFrame(current_path_.poses.back());
        // RCLCPP_WARN(  
        //   logger_, "sampled_pt_base(%.2f, %.2f, %.2f)",
        //   sampled_pt_base.position.x, sampled_pt_base.position.y, tf2::getYaw(sampled_pt_base.orientation));          
        angular_distance_to_heading = tf2::getYaw(sampled_pt_base.orientation);
        temp_angular_threshold = yaw_goal_tolerance_;       
      } else {
      ...
      */
      ...
      geometry_msgs::msg::Pose sampled_pt_base = transformPoseToBaseFrame(getSampledPathPt(pose));
      // RCLCPP_WARN(  
      //   logger_, "sampled_pt_base(%.2f, %.2f, %.2f)",
      //   sampled_pt_base.position.x, sampled_pt_base.position.y, tf2::getYaw(sampled_pt_base.orientation));
      ...
      // RCLCPP_WARN(  
      //   logger_,
      //   "angular_distance_to_heading : %.2frad, temp_angular_threshold : %.2frad.",
      //   angular_distance_to_heading, temp_angular_threshold);
...
geometry_msgs::msg::PoseStamped RotationShimController::getSampledPathPt(const geometry_msgs::msg::PoseStamped & robot_pose)
{
  // double path_len = nav2_util::geometry_utils::calculate_path_length(current_path_);
  // if (path_len < xy_goal_tolerance_) {
  //   RCLCPP_WARN(
  ...
  double forward_sampling_distance = forward_sampling_distance_;
  // if (path_len < forward_sampling_distance_) {
  //   forward_sampling_distance = path_len * 0.5;
  // }
...
RotationShimController::transformPoseToBaseFrame(const geometry_msgs::msg::PoseStamped & pt)
{
  ...
  if (!nav2_util::transformPoseInTargetFrame(
    pt, pt_base, *tf_, costmap_ros_->getBaseFrameID())) {
