...
#include <memory>
#include <limits>
#include "nav2_bt_navigator/navigators/navigate_to_lift.hpp"

namespace nav2_bt_navigator
{

bool
NavigateToLiftNavigator::configure(
  ...
  std::shared_ptr<nav2_util::OdomSmoother> odom_smoother)
{
  ...
  if (!node->has_parameter("navi_type_topic")) {
    node->declare_parameter("navi_type_topic", std::string("/navi_type"));
  }

  navi_type_topic_ = node->get_parameter("navi_type_topic").as_string();
  ...
  goal_sub_ = node->create_subscription<geometry_msgs::msg::PoseStamped>(
    ...
    std::bind(&NavigateToLiftNavigator::onGoalPoseReceived, this, std::placeholders::_1));

  navi_type_sub_ = node->create_subscription<std_msgs::msg::Int32>(
        navi_type_topic_, rclcpp::SystemDefaultsQoS(),
        std::bind(&NavigateToLiftNavigator::naviTypeCallback, this, std::placeholders::_1));  // add

  return true;
}

std::string
NavigateToLiftNavigator::getDefaultBTFilepath(
  rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node)
{
  std::string default_bt_xml_filename;
  auto node = parent_node.lock();

  if (!node->has_parameter("default_nav_to_lift_bt_xml")) {
    std::string pkg_share_dir =
      ament_index_cpp::get_package_share_directory("nav2_bt_navigator");
    node->declare_parameter<std::string>(
      "default_nav_to_lift_bt_xml",
      pkg_share_dir +
      "/behavior_trees/navigate_to_pose_w_replanning_and_recovery.xml");
  }

  node->get_parameter("default_nav_to_lift_bt_xml", default_bt_xml_filename);

  return default_bt_xml_filename;
}

bool
NavigateToLiftNavigator::cleanup()
{
  goal_sub_.reset();
  self_client_.reset();
  return true;
...
bool
NavigateToLiftNavigator::goalReceived(ActionT::Goal::ConstSharedPtr goal)
{
...
void
NavigateToLiftNavigator::goalCompleted(typename ActionT::Result::SharedPtr /*result*/)
{
}

void
NavigateToLiftNavigator::onLoop()
{
...
void
NavigateToLiftNavigator::onPreempt(ActionT::Goal::ConstSharedPtr goal)
{
...
void
NavigateToLiftNavigator::initializeGoalPose(ActionT::Goal::ConstSharedPtr goal)
{
  RCLCPP_INFO(
...
void
NavigateToLiftNavigator::onGoalPoseReceived(const geometry_msgs::msg::PoseStamped::SharedPtr pose)
{
  ActionT::Goal goal;
  goal.pose = *pose;
...
void NavigateToLiftNavigator::naviTypeCallback(const std_msgs::msg::Int32::ConstSharedPtr msg)  // add
{
    auto blackboard = bt_action_server_->getBlackboard();
    blackboard->set<int>("navi_type", msg->data);

    RCLCPP_INFO(logger_, "Blackboard set navi_type : %d", msg->data);
}
