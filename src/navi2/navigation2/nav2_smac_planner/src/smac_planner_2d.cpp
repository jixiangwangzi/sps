...
: _a_star(nullptr),
  ...
  smoother_type_("nav2_smoother::SimpleSmoother"),
  _costmap(nullptr),
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  nav2_util::declare_parameter_if_not_declared(
    node, name + ".smoother.costmap_topic", rclcpp::ParameterValue(
      std::string("global_costmap/costmap_raw")));
  nav2_util::declare_parameter_if_not_declared(
    node, name + ".smoother.footprint_topic", rclcpp::ParameterValue(
      std::string("global_costmap/published_footprint")));
  nav2_util::declare_parameter_if_not_declared(
    node, name + ".smoother.robot_base_frame",
    rclcpp::ParameterValue(std::string("base_link")));
  nav2_util::declare_parameter_if_not_declared(
    node, name + ".smoother.transform_tolerance", rclcpp::ParameterValue(0.1));
  nav2_util::declare_parameter_if_not_declared(
    node, name + ".smoother.smoother_plugin", 
    rclcpp::ParameterValue(std::string("nav2_smoother::SimpleSmoother")));
  std::string costmap_topic, footprint_topic, robot_base_frame;
  double transform_tolerance;
  node->get_parameter(name + ".smoother.costmap_topic", costmap_topic);
  node->get_parameter(name + ".smoother.footprint_topic", footprint_topic);
  node->get_parameter(name + ".smoother.transform_tolerance", transform_tolerance);
  node->get_parameter(name + ".smoother.robot_base_frame", robot_base_frame);
  node->get_parameter(name + ".smoother.smoother_plugin", smoother_type_);
  ...
  footprint_sub_ = std::make_shared<nav2_costmap_2d::FootprintSubscriber>(
    node, footprint_topic, 1.0);
  if (!smoother_type_.empty())
  {
    smoother_ =
        lp_loader_.createUniqueInstance(smoother_type_);
    smoother_->configure(node, name + ".smoother", tf, costmap_sub_, footprint_sub_);
  } else {
    smoother_ = nullptr;
  }
  ...
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  // Smooth plan
  if (plan.poses.size() > 6 && smoother_ != nullptr) {
    smoother_->smooth(plan, rclcpp::Duration::from_seconds(2000.0));   
  ...
  for (auto & changed_parameter : event->changed_parameters) {
    ...
    if (type == ParameterType::PARAMETER_DOUBLE) {
      ...
      if (name == _name + ".downsample_costmap") {
        ...
        RCLCPP_WARN(
          _logger,
          "_use_final_approach_orientation : %d.", _use_final_approach_orientation);
      }
