...
ClearAppointedCostmapService::ClearAppointedCostmapService(
  const std::string & service_node_name,
  const BT::NodeConfiguration & conf)
: BtServiceNode<nav2_msgs::srv::ClearEntireCostmap>(service_node_name, conf)
{
...
void ClearAppointedCostmapService::on_tick()
{
  increment_recovery_count();
}
...
{
  factory.registerNodeType<nav2_behavior_tree::ClearEntireCostmapService>(
    "ClearEntireCostmap");
  ...
  factory.registerNodeType<nav2_behavior_tree::ClearAppointedCostmapService>(
    "ClearAppointedCostmap");
}
