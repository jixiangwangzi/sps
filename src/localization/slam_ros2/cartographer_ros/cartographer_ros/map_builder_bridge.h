...
class MapBuilderBridge {
 public:
  ...
  // int LoadMap(const std::string map_path, std::shared_ptr<nav_msgs::OccupancyGrid>& occ_map);
  bool SetOccMap(const nav_msgs::msg::OccupancyGrid& occ_map); 
  bool FilterMap(Eigen::MatrixXd& map_img);
  int AddTrajectory(
      const std::set<
  ...
  GetTrajectoryStates();
  int GetTrajectoryNum(){ return map_builder_->num_trajectory_builders(); };
