...
std::unique_ptr<carto::sensor::OdometryData> SensorBridge::ToOdometryData(
  ...
  Eigen::Vector3d line_velocity(msg->twist.twist.linear.x, msg->twist.twist.linear.y, msg->twist.twist.linear.z);
  ...
  return absl::make_unique<carto::sensor::OdometryData>(
      carto::sensor::OdometryData{
          time, ToRigid3d(msg->pose.pose) * sensor_to_tracking->inverse(), line_velocity, angular_velocity});
}
...
void SensorBridge::HandleOdometryMessage(
  ...
  if (odometry_data != nullptr) {
    trajectory_builder_->AddSensorData(
        sensor_id,
        carto::sensor::OdometryData{odometry_data->time, odometry_data->pose, odometry_data->line_velocity, odometry_data->angular_velocity});
  }
}
...
void SensorBridge::HandleTagToRobotMessage(
    const std::string& sensor_id,
    const geometry_msgs::msg::PoseStamped::ConstSharedPtr& msg) {
  cartographer::sensor::LandmarkData landmark_data;
  landmark_data.time = FromRos((*msg).header.stamp);
  landmark_data.landmark_observations.push_back(
      {(*msg).header.frame_id, ToRigid3d((*msg).pose),
        1000, 1000});
  LOG(INFO) << "SensorBridge::HandleTagToRobotMessage \n";
  // auto tracking_from_landmark_sensor = tf_bridge_.LookupToTracking(
  //     landmark_data.time, CheckNoLeadingSlash(msg->header.frame_id));
  // if (tracking_from_landmark_sensor != nullptr) {
    // for (auto& observation : landmark_data.landmark_observations) {
