...
LaserScanToPointCloudWithIntensities(const LaserMessageType& msg) {
  ...
  float kFOV = (msg.angle_max-msg.angle_min)*180/3.14;
  
  PointCloudWithIntensities point_cloud;
  float angle = msg.angle_min;
  float time_increment = 0.0;
  float msg_time_increment = 0.0;
  if( std::fabs( msg.time_increment * (kFOV * 4 - 1) - msg.scan_time) < 0.00001){
    msg_time_increment =  msg.time_increment * (kFOV * 4 - 1) / (360 * 4 - 1);
  }
  else{
    msg_time_increment =  msg.time_increment;
  }
  ...
  for (unsigned int  i = 0; i < msg.ranges.size(); ++i) {
    ...
    if (HasEcho(echoes)) {
      ...
      if (msg.range_min <= first_echo && first_echo <= msg.range_max) {
        ...
        else if(i < (kFOV / 2 * 4 + 30)){
          time_increment = (kFOV / 2 * 4 - i) * msg_time_increment;
        }
        else{
          time_increment = (kFOV / 2 * 4 - i + 360 * 4) * msg_time_increment;
        }
        // time_increment = i * msg_time_increment;
  ...
  std::sort(point_cloud.points.begin(), point_cloud.points.end(),
            [](const cartographer::sensor::TimedRangefinderPoint& a,
               const cartographer::sensor::TimedRangefinderPoint& b) {
...
Rigid2d::Rotation2D ToRotation2D(const Eigen::Quaterniond& quaternion) {
  return Rigid2d::Rotation2D(GetYaw(quaternion));
}

Rigid2d ToRigid2d(const geometry_msgs::msg::TransformStamped& transform) {
  return Rigid2d(ToEigen(transform.transform.translation).head<2>(),
                 GetYaw(ToEigen(transform.transform.rotation)));
}

Rigid2d ToRigid2d(const geometry_msgs::msg::Pose& pose) {
  return Rigid2d({pose.position.x, pose.position.y},
                 GetYaw(ToEigen(pose.orientation)));
}
