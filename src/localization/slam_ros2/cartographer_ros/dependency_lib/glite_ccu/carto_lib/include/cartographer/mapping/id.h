...
class MapById {
 ...
 public:
  ...
  void Insert(const IdType& id, const DataType& data) {
    ...
    if(!trajectory.data_.emplace(GetIndex(id), data).second){
      LOG(ERROR) << "trajectory.data_.emplace(GetIndex(id), data) fail";
    }
  }

  // Removes the data for 'id' which must exist.
  void Trim(const IdType& id) {
    if(trajectories_.find(id.trajectory_id) == trajectories_.end()){
      return;
    }
    auto& trajectory = trajectories_.at(id.trajectory_id);
    const auto it = trajectory.data_.find(GetIndex(id));
    if(it == trajectory.data_.end()){
      return;
    }
