...
class MapBuilderInterface {
 public:
  ...
  virtual int AddTrajectoryForLoadMap(const std::string path, const Eigen::MatrixXd& img) = 0;
  virtual int AddTrajectoryForLoadMap(const Eigen::Vector2f& origin, const Eigen::MatrixXd& img) = 0;
  ...
  virtual std::shared_ptr<double> getScorePoseFromSubmapPtr()  { return nullptr;  }
  
  virtual std::shared_ptr<double> getScorePoseFromMapPtr()  { return nullptr;  }

  virtual bool setGlobalMessagePtr(std::shared_ptr<cartographer::GlobalMessage>& globalMessagePtr){
    if(globalMessagePtr == nullptr){
        LOG(ERROR) << "MapBuilderInterface:: globalMessagePtr is nullptr";
        return false;
    }
