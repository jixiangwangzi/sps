...
class Grid2D : public GridInterface {
 public:
  ...
  float GetCorrespondenceCost(const Eigen::Array2i& cell_index) const {
    ...
    float correspondence_cost = (*value_to_correspondence_cost_table_)
        [correspondence_cost_cells()[ToFlatIndex(cell_index)]];
    if(std::fabs(correspondence_cost - 0.5f) < 0.00001){
  ...
  float GetCorrespondenceCostSelf(const Eigen::Array2i& cell_index) const {
    if (!limits().Contains(cell_index)) return unknown_result_;
    return (*value_to_correspondence_cost_table_self_)
        [correspondence_cost_cells()[ToFlatIndex(cell_index)]];
  }
  ...
  /// 一维索引转二维像素索引
  Eigen::Array2i ToXYIndex(const int& index) {
    CHECK(index < correspondence_cost_cells_.size() && index >= 0) << index;
    int x = index % limits_.cell_limits().num_x_cells;
    int y = index / limits_.cell_limits().num_x_cells;
    return Eigen::Array2i(x, y);
  }
...
//  private:
  ...
  float unknown_result_;
  // Bounding box of known cells to efficiently compute cropping limits.
  Eigen::AlignedBox2i known_cells_box_;
  const std::vector<float>* value_to_correspondence_cost_table_;
  const std::vector<float>* value_to_correspondence_cost_table_self_;
};
