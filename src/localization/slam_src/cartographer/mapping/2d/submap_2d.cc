 ...
 * @brief 栅格图转换为点云数据
 * 
 * @param correspondence_cost 空闲概率
 * @return 点云(local系)
 */
std::vector<sensor::RangefinderPoint> Submap2D::GetRangeData(const float& correspondence_cost) const
{
    std::vector<sensor::RangefinderPoint> points;
    
    for(size_t i = 0; i < grid_->correspondence_cost_cells_.size(); i++)
    {
        Eigen::Array2i cell_index = grid_->ToXYIndex(i);
        if(grid_->GetCorrespondenceCost(cell_index) < correspondence_cost) /// 找到空闲概率小于阈值的点
        {
            Eigen::Vector2f xy = grid_->limits().GetCellCenter(cell_index);
            points.push_back(sensor::RangefinderPoint{Eigen::Vector3f{xy[0], xy[1], 0}});
        }
    }
    // LOG(INFO) << "for correspondence_cost < " << correspondence_cost << ",sub map points size=" << points.size() << " --------";
    return points;
}
