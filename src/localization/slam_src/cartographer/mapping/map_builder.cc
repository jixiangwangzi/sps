...
MapBuilder::MapBuilder(const proto::MapBuilderOptions& options)
  ...
  if (options.use_trajectory_builder_2d()) {
    pose_graph_ = absl::make_unique<PoseGraph2D>(
        ...
        &thread_pool_, globalMessagePtr_);
  }
  ...
  if (options_.use_trajectory_builder_3d()) {
    ...
    trajectory_builders_.push_back(absl::make_unique<CollatedTrajectoryBuilder>(
        ...
        CreateGlobalTrajectoryBuilder3D(
            ...
            local_slam_result_callback, pose_graph_odometry_motion_filter), globalMessagePtr_));
  } else {
    ...
    trajectory_builders_.push_back(absl::make_unique<CollatedTrajectoryBuilder>(
        ...
        CreateGlobalTrajectoryBuilder2D(
            ...
            local_slam_result_callback, pose_graph_odometry_motion_filter), globalMessagePtr_));
    auto collatedTrajectoryBuilder = (trajectory_builders_.back()).get();
    scorePoseFromSubmapPtr_ = collatedTrajectoryBuilder->getScorePoseFromSubmapPtr();
    scorePoseFromMapPtr_ = collatedTrajectoryBuilder->getScorePoseFromMapPtr();
  }
...
int MapBuilder::AddTrajectoryForLoadMap(const std::string path, const Eigen::MatrixXd& img){
    const int trajectory_id = trajectory_builders_.size();
    trajectory_builders_.emplace_back();
    proto::TrajectoryBuilderOptionsWithSensorIds options_with_sensor_ids_proto;
    all_trajectory_builder_options_.push_back(options_with_sensor_ids_proto);
    ...
    //调试写死
    Eigen::Vector2f origin(-9.97089, -7.97386);
    Eigen::Vector2d kInitialSubmapSize(img.rows(),img.cols());
    Eigen::Vector2d kInitialSubmapSize2(img.cols(),img.rows());
    double resolution = 0.05;
    
    // auto grid = std::make_unique<ProbabilityGrid>(
    //       MapLimits(resolution,
    //                 origin.cast<double>() + 0.5 * kInitialSubmapSize *
    //                                             resolution *
    //                                             Eigen::Vector2d::Ones(),
    //                 CellLimits(1458, 1214)),
    //       &conversion_tables);
    auto grid = std::make_unique<ProbabilityGrid>(
      MapLimits(resolution,
                origin.cast<double>() + resolution *
                                            kInitialSubmapSize2,
                CellLimits(img.rows(),img.cols())),
      &conversion_tables);
    for (uint32_t i = 0; i < img.rows(); i++) {
      for (uint32_t j = 0; j < img.cols(); j++) {
        Eigen::Array2i xy_index(i, j);
        // int index = (img.rows()-i-1) * img.cols() + j;
        uint8_t pixel = img(i, img.cols() - j -1);
        if (pixel < 100) {
          grid->SetProbability(xy_index, 0.9);
        } else if (pixel > 180) {
          grid->SetProbability(xy_index, 0.1);
        } else {
          grid->SetProbability(xy_index, 0.5);
        }
      }
    }

    transform::Rigid3d origin_;
    pose_graph_->AddSubmapFromMap(origin_, std::move(grid));
    return trajectory_id;
}

int MapBuilder::AddTrajectoryForLoadMap(const Eigen::Vector2f& origin, const Eigen::MatrixXd& img){
    const int trajectory_id = trajectory_builders_.size();
    trajectory_builders_.emplace_back();
    proto::TrajectoryBuilderOptionsWithSensorIds options_with_sensor_ids_proto;
    all_trajectory_builder_options_.push_back(options_with_sensor_ids_proto);
    ...
    Eigen::Vector2d kInitialSubmapSize(img.cols(),img.rows());
    double resolution = 0.05;
    ...
    for (uint32_t i = 0; i < img.rows(); i++) {
      for (uint32_t j = 0; j < img.cols(); j++) {
        ...
        if (pixel < 50) {
          ...
          grid->SetProbability(xy_index, 0.8);
        }
        else if(pixel < 150){
          grid->SetProbability(xy_index, 0.5);
        }
        else if(pixel < 220){
          grid->SetProbability(xy_index, 0.3);
        }
        else if(pixel <= 255){
          grid->SetProbability(xy_index, 0.1);
        }
...
std::map<int, int> MapBuilder::LoadState(
  ...
  std::cout << "enter MapBuilder::LoadState(...)!!!"<<std::endl;
...
bool MapBuilder::setGlobalMessagePtr(std::shared_ptr<cartographer::GlobalMessage>& globalMessagePtr){
  if(globalMessagePtr == nullptr){
      LOG(ERROR) << "MapBuilder:: globalMessagePtr_ is nullptr";
      return false;
  }
  if(pose_graph_ == nullptr){
      LOG(ERROR) << "MapBuilder:: pose_graph_ is nullptr";
      return false;
  }
  pose_graph_->setGlobalMessagePtr(globalMessagePtr);
