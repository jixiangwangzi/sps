...
class PoseGraph2D : public PoseGraph {
 public:
  ...
  void AddSubmapFromMap(const transform::Rigid3d& global_pose,
                             std::unique_ptr<ProbabilityGrid> grid) override;                       
  ...
  std::shared_ptr<double> getScorePoseFromMapPtr(){
      ...
      LOG(ERROR) << "pose_graph_2d.scorePosePtr_ is null"; 
      }
  ...
  bool setGlobalMessagePtr(std::shared_ptr<cartographer::GlobalMessage>& globalMessagePtr) override{
    if(globalMessagePtr == nullptr){
        LOG(ERROR) << "PoseGrapher2d:: globalMessagePtr is nullptr";
        return false;
    }
    globalMessagePtr_ = globalMessagePtr;
    constraint_builder_.setGlobalMessagePtr(globalMessagePtr_);
    return true;
  }
 private:
