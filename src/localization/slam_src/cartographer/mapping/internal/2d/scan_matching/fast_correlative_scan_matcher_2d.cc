...
CreateFastCorrelativeScanMatcherOptions2D(
  ...
  options.set_linear_search_window_init(
      parameter_dictionary->GetDouble("linear_search_window_init"));
  options.set_angular_search_window_init(
      parameter_dictionary->GetDouble("angular_search_window_init"));
  options.set_linear_search_window_init_no_match(
      parameter_dictionary->GetDouble("linear_search_window_init_no_match"));
  options.set_angular_search_window_init_no_match(
      parameter_dictionary->GetDouble("angular_search_window_init_no_match"));
...
PrecomputationGrid2D::PrecomputationGrid2D(
  ...
  // LOG(INFO) << "limits.num_x_cells: "<<limits.num_x_cells<<" limits.num_y_cells: "<< limits.num_y_cells;
  // LOG(INFO) << "width: "<<width<<" stride: "<< stride;
...
bool FastCorrelativeScanMatcher2D::Match(
    ...
    const double& linearCumulateSinceLost,
    const double& angularCumulateSinceLost,
    const float min_score, float* score,
    transform::Rigid2d* pose_estimate) const {
  // LOG(INFO) << "linearCumulateSinceLost: " << linearCumulateSinceLost<<" m" << "angularCumulateSinceLost: " << 180 * angularCumulateSinceLost << " degree";
  auto linear_search_window = options_.linear_search_window() + 0.1 * linearCumulateSinceLost;
  auto angular_search_window = options_.angular_search_window() + 0.005 * angularCumulateSinceLost;
  if(linear_search_window < 0.05){
    linear_search_window = 0.05;
  }
  if(angular_search_window < 0.0314){
    angular_search_window = 0.0314;
  }
  if(angular_search_window > (3.14 / 2)){
    angular_search_window = (3.14 / 2);
  }
  // LOG(INFO) << "linear_search_window: " << linear_search_window << " m " 
            // << "angular_search_window: " << 180 * angular_search_window / 3.14 << " degree";
...
bool FastCorrelativeScanMatcher2D::MatchInitMap(
    const transform::Rigid2d& initial_pose_estimate,
    const sensor::PointCloud& point_cloud, const float min_score, float* score,
    transform::Rigid2d* pose_estimate) const {
  const SearchParameters search_parameters(options_.linear_search_window_init(),
                                           options_.angular_search_window_init(),
                                           point_cloud, 0.05);
  // LOG(INFO) << "linear_search_window: " << options_.linear_search_window_init()
  //           << " angular_search_window: " << options_.linear_search_window_init()
  //           << " min_score: " << min_score;
  bool result =  MatchWithSearchParameters(search_parameters, initial_pose_estimate,
                                   point_cloud, min_score, score,
                                   pose_estimate);
  LOG(INFO) << "MatchInitMap result pose: " << *pose_estimate << ", with score: " << *score;
  return result;
}

bool FastCorrelativeScanMatcher2D::MatchInitMapNoMatch(
    const transform::Rigid2d& initial_pose_estimate,
    const sensor::PointCloud& point_cloud, const float min_score, float* score,
    transform::Rigid2d* pose_estimate) const {
  const SearchParameters search_parameters(options_.linear_search_window_init_no_match(),
                                           options_.angular_search_window_init_no_match(),
                                           point_cloud, 0.05);
  // LOG(INFO) << "linear_search_window: " << options_.linear_search_window_init_no_match()
  //           << "angular_search_window: " << options_.angular_search_window_init_no_match()
  //           << " min_score: " << min_score;
  bool result =  MatchWithSearchParameters(search_parameters, initial_pose_estimate,
                                   point_cloud, min_score, score,
                                   pose_estimate);
  LOG(INFO) << "MatchInitMapNoMatch result pose: " << *pose_estimate << ", with score: " << *score;
  return result;
}
