...
sensor::TimedPointCloudOriginData RangeDataCollator::AddSingleRangeData(
    const std::string& sensor_id,
    sensor::TimedPointCloudData timed_point_cloud_data) {
  // LOG(INFO) << "AddSingleRangeData 1";
  sensor::TimedPointCloudOriginData result{timed_point_cloud_data.time, {}, {}};
  std::size_t origin_index = result.origins.size();
  result.origins.push_back(timed_point_cloud_data.origin);
  // LOG(INFO) << "AddSingleRangeData 2";
  // int i = 0;
  for(auto range : timed_point_cloud_data.ranges){
      sensor::TimedPointCloudOriginData::RangeMeasurement point{
          range, 47.0f, origin_index};
      result.ranges.push_back(point);
      // std::cout << "point.time: " << point.point_time.time << "  i:" << i++ << std::endl;
  }
  // LOG(INFO) << "AddSingleRangeData 3";
  std::sort(result.ranges.begin(), result.ranges.end(),
            [](const sensor::TimedPointCloudOriginData::RangeMeasurement& a,
               const sensor::TimedPointCloudOriginData::RangeMeasurement& b) {
              return a.point_time.time < b.point_time.time;
