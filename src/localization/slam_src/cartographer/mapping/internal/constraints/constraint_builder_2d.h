...
class ConstraintBuilder2D {
 public:
  ...
  void MaybeAddConstraint(const Submap2D* src_submap, const SubmapId& tgt_submap_id, const Submap2D* tgt_submap,
  ...
  void MaybeAddInitConstraint(const Submap2D* src_submap, const SubmapId& tgt_submap_id, const Submap2D* tgt_submap,
                          const NodeId& node_id,
                          const TrajectoryNode::Data* const constant_data,
                          const transform::Rigid2d& initial_relative_pose,
                          const transform::Rigid2d& global_pose_2d);
  ...
  std::shared_ptr<double> getScorePoseFromMapPtr(){
      if(scorePoseFromMapPtr_ == nullptr){
      LOG(ERROR) << "constraint_builder_2d.scorePosePtr_ is null"; 
      }
      return scorePoseFromMapPtr_;
  }
  bool setGlobalMessagePtr(std::shared_ptr<cartographer::GlobalMessage>& globalMessagePtr){
    if(globalMessagePtr == nullptr){
        LOG(ERROR) << "constraint_builder_2h:: globalMessagePtr is nullptr";
        return false;
    }
 ...
 private:
  ...
  bool maybeLongCorridor_;
  bool initFinish_;
  double linearCumulateSinceLost_;
  double angularCumulateSinceLost_;
  transform::Rigid2d globalPose2dLastLost_;
  bool poseLostInit_;
  std::shared_ptr<double> scorePoseFromMapPtr_;
