...
#include "cartographer/mapping/internal/2d/scan_matching/occupied_space_cost_function_2d_self.h"
...
void OptimizationProblem2D::Solve(
    ...
    const std::map<int, PoseGraphInterface::TrajectoryState>&
        trajectories_state,
    const std::map<std::string, LandmarkNode>& landmark_nodes, const bool& is_localization) {
  if (node_data_.empty()) {
    // Nothing to optimize.
    return;
  }
  // LOG(INFO)<<"start OptimizationProblem2D::Solve !!!";
  ...
  SubmapId minSubmap(1, 2147483641);
  for (const auto& submap_id_data : submap_data_) {
    ...
    // LOG(INFO) << "OptimizationProblem2D::Solve submap_id_data.id: " << submap_id_data.id;
    if(submap_id_data.id.trajectory_id > 0 && submap_id_data.id.submap_index <= minSubmap.submap_index){
        minSubmap = submap_id_data.id;
        // LOG(INFO) << "OptimizationProblem2D::Solve minSubmap.id: " << minSubmap;
    }
  ...
  bool flag_has_global_constraints = false;
  //如果子图和地图间没有约束，则固定第一个子图
  bool contraintBetweenDifferentTrajectory = false;
  for (const Constraint& constraint : constraints) {
    if(constraint.node_id.trajectory_id != constraint.submap_id.trajectory_id){
      contraintBetweenDifferentTrajectory = true;
    }
    if(constraint.pose.rotation_weight > 1e6){
      flag_has_global_constraints = true;
    }
  }
  if(!contraintBetweenDifferentTrajectory && frozen_trajectories.size() > 0){
    // LOG(INFO) << "SetSubmapConstant Submap id: " << minSubmap;
    problem.SetParameterBlockConstant(C_submaps.at(minSubmap).data());
  }
  ...
  for (const Constraint& constraint : constraints) {
    ...
    //   LOG(INFO) << "optimization constraint node id:" << constraint.node_id << " submap id :" << constraint.submap_id 
    //           << " pose: \n" << constraint.pose.zbar_ij << "\n yaw: " << cartographer::transform::GetYaw(constraint.pose.zbar_ij) * 180. / 3.1415926;
    // }
  ...
  if(is_localization){
    for (auto trajectory_node : trajectory_nodes) {
      if(C_nodes.Contains(trajectory_node.id)){
        problem.AddResidualBlock(
            CreateOccupiedSpaceCostFunction2DSelf(
                // 500 /
                //     std::sqrt(static_cast<double>(trajectory_node.data.constant_data->filtered_gravity_aligned_point_cloud.size())),
                100.0,
                trajectory_node.data.constant_data->filtered_gravity_aligned_point_cloud, *map_ptr->grid()),
            nullptr /* loss function */, C_nodes.at(trajectory_node.id).data());
      }
    }
  }
  ...
  for (auto node_it = node_data_.begin(); node_it != node_data_.end();) {
    ...
    for (++node_it; node_it != trajectory_end; ++node_it) {
      ...
      //   problem.AddResidualBlock(
      //       CreateAutoDiffSpaCostFunction(Constraint::Pose{
      //           *relative_odometry, options_.odometry_translation_weight() / 100,
      //           options_.odometry_rotation_weight()/100}),
  ...
  //  LOG(INFO)<<"end OptimizationProblem2D::Solve !!!";
}
