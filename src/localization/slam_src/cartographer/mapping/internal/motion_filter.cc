...
bool MotionFilter::IsSimilar(const common::Time time, const transform::Rigid3d& pose, const bool& by_time) {
  ++num_total_;
  if(by_time)
  {
  ...
  {
    if (num_total_ > 1 && (pose.translation() - last_pose_.translation()).norm() <= options_.max_distance_meters() &&
        transform::GetAngle(pose.inverse() * last_pose_) <= options_.max_angle_radians()) {
        return true;
    }
  }
