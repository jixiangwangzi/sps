...
STARTUP_LOG_FILE=${log_path}/${current_day}/localization/${current_date}/startup.log
...
mkdir -p ${log_path}/${current_day}/localization/${current_date}/
ln -snf ${log_path}/${current_day}/localization/${current_date} /root/latest
...
if [ "$ROBOT_MODEL" = "patrol" ]; then
  ${UN_BUF} ros2 launch -n -a lidar_slam patrol_lidar_slam.launch.py  log-level:=INFO > ${log_path}/${current_day}/localization/${current_date}/lidar_slam.log 2>&1 &
elif [ "$ROBOT_MODEL" = "ginger_lite" ]; then
  ${UN_BUF} ros2 launch -n -a lidar_slam lidar_slam.launch.py  log-level:=INFO > ${log_path}/${current_day}/localization/${current_date}/lidar_slam.log 2>&1 &
else
  ${UN_BUF} ros2 launch -n -a lidar_slam lidar_slam.launch.py  log-level:=INFO > ${log_path}/${current_day}/localization/${current_date}/lidar_slam.log 2>&1 &
fi
sleep 1
...
log_info "startup localization complete"
